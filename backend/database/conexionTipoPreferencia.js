...
class ConexionTipopreferencia {
    ...
    registrarTipopreferencia = async(body) => {
        let resultado = 0;
        this.conectar();
        try{
            // const usuarioNuevo = new Persona(body); //Con esto añade los timeStamps.
            // await usuarioNuevo.save();
            const usuarioNuevo = await models.Tipopreferencia.create(body);
    ...
    modificarTiporelacion = async(id, body) => {
        this.conectar();
    ...
    borrarTipopreferencia = async(id) => {
        this.conectar();
        let resultado = await models.Tipopreferencia.findByPk(id);
        if (!resultado){
            this.desconectar();
