...
class ConexionNino {
    ...
    getNino = async(id) => {
        let resultado = [];
        this.conectar();
        resultado = await models.Nino.findByPk(id);
        this.desconectar();
        if (!resultado){
            throw error;
        }
    ...
    registrarNino = async(body) => {
        let resultado = 0;
        this.conectar();
        try{
            // const usuarioNuevo = new Persona(body); //Con esto añade los timeStamps.
            // await usuarioNuevo.save();
            const usuarioNuevo = await models.Nino.create(body);
    ...
    modificarNino = async(id, body) => {
        this.conectar();
        let resultado = await models.Nino.findByPk(id);
        if (!resultado){
            this.desconectar();
            throw error;
        }
