...
class ConexionNinopreferencia {
    ...
    registrarNinopreferencia = async(body) => {
        let resultado = 0;
        this.conectar();
        try{
            // const usuarioNuevo = new Persona(body); //Con esto añade los timeStamps.
            // await usuarioNuevo.save();
            const usuarioNuevo = await models.Ninopreferencia.create(body);
    ...
    modificarNinopreferencia = async(id, body) => {
        this.conectar();
    ...
    borrarNinopreferencia = async(id) => {
        this.conectar();
        let resultado = await models.Ninopreferencia.findByPk(id);
        if (!resultado){
            this.desconectar();
