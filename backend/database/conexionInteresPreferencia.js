...
class ConexionInterespreferencia {
    ...
    getInterespreferencia = async(id) => {
        let resultado = [];
        this.conectar();
    ...
    registrarInterespreferencia = async(body) => {
        let resultado = 0;
        this.conectar();
        try{
            // const usuarioNuevo = new Persona(body); //Con esto añade los timeStamps.
            // await usuarioNuevo.save();
            const usuarioNuevo = await models.Interespreferencia.create(body);
    ...
    modificarInterespreferencia = async(id, body) => {
        this.conectar();
    ...
    borrarInterespreferencia = async(id) => {
        this.conectar();
        let resultado = await models.Interespreferencia.findByPk(id);
        if (!resultado){
            this.desconectar();
