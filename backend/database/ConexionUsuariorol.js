...
class ConexionUsuariorol {
    ...
    getUsuarioRol = async(id) => {
        let resultado = [];
        this.conectar();
        resultado = await models.usuariorol.findByPk(id);
        this.desconectar();
        if (!resultado){
            throw error;
        }
    ...
    registrarUsuarioRol = async(body) => {
        let resultado = 0;
        this.conectar();
        try{
            // const usuarioNuevo = new Persona(body); //Con esto añade los timeStamps.
            // await usuarioNuevo.save();
            const usuarioNuevo = await models.usuariorol.create(body);
    ...
    modificarUsuarioRol = async(id, body) => {
        this.conectar();
...
module.exports = ConexionUsuariorol;